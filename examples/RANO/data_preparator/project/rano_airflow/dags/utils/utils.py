...
def create_documentation_for_manual_steps(subject_slash_timepoint):
    ...
    path of **{CONFIRMED_TUMOR_EXTRACTION_FILE}**) as is. Once the finalized Tumor Segmentation is detected, the pipeline will automatically proceed to the next stage.
    ...
    have completed). If the Brain mask itself needs corrections, please refer to the **BRAIN MASK REVIEW** section in the DAG documentation."""
    ...
    Please review the automatically generated Tumor Segmentation by the pipeline. If necessary, the automatically generated Brain Mask can also be corrected.
