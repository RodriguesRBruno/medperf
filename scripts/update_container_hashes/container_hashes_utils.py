...
def get_container_info(
    ...
    if not hashes_matched:
        print(
            f"Hash mismatch when downloading assets from Container ID {container_id}!\n"
...
def get_container_hashes(
    ...
    if not exclude_synapse:
        ...
        try:
            print(
                "Logging into Synapse with existing credentials (ie from medperf auth synapse login)"
            )
    ...
    for container_id, container_info in container_id_to_container.items():
        ...
        this_container_yaml, yaml_hash_matched = get_container_info(
        ...
        if this_container_yaml is not None:
            ...
            new_hash = get_docker_hash(
                docker_client=docker_client,
                container_dict=this_container_yaml,
                container_id=container_id,
            )
...
def get_container_jsons(
    ...
    for container in containers:
        ...
        if (is_synapse and not include_synapse_links) or (
            not is_synapse and not include_public_links
        ):
            continue
